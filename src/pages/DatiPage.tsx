...
import {
  ...
  ChevronDown, ChevronUp, CreditCard, AlertCircle, Layers, Eye, EyeOff
} from 'lucide-react';
import {
...
export default function DatiPage({ property }: { property: Property }) {
  ...
  const [showBreakdownR, setShowBreakdownR] = useState(false);  // toggle rate per unità
  const [showBreakdownS, setShowBreakdownS] = useState(false);  // toggle spese per unità
  const [showBreakdownC, setShowBreakdownC] = useState(false);  // toggle consumi per unità
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
              ...
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
                ...
                <tbody>
                  {riassunto.map((r,i)=>{
                    ...
                    const prvTotSaldo = prv&&prv.sC!==null&&prv.sB!==null&&prv.sCa!==null?prv.sC+prv.sB+prv.sCa:null;
                    return (
                      <tr key={r.anno} style={{ borderBottom:'1px solid var(--border)', background:i%2===0?'#fff':'var(--bg3)' }}>
                        ...
                        <td style={{ padding:'8px 10px', textAlign:'right' }}>
                          {r.sf!=null?<><span style={{ fontWeight:700 }}>€{f0(r.sf)}</span>{prv?.sf!=null&&<div style={{ fontSize:9, fontWeight:700, color:r.sf>prv.sf!'var(--red)':'var(--green)' }}>{r.sf>prv.sf!'▲':'▼'}{Math.abs(pct(r.sf,prv.sf!)).toFixed(1)}%</div>}</>:<span style={{ color:'var(--text3)' }}>—</span>}
                        </td>
                        {/* Consumi */}
                        <td style={{ padding:'8px 10px', textAlign:'right' }}>
                          {r.con!=null?<><span style={{ fontWeight:700 }}>€{f0(r.con)}</span>{prv?.con!=null&&<div style={{ fontSize:9, fontWeight:700, color:r.con>prv.con!'var(--red)':'var(--green)' }}>{r.con>prv.con!'▲':'▼'}{Math.abs(pct(r.con,prv.con!)).toFixed(1)}%</div>}</>:<span style={{ color:'var(--text3)' }}>—</span>}
                        </td>
                        {/* Tot spese */}
                        <td style={{ padding:'8px 10px', textAlign:'right' }}>
                          {r.tot!=null?<><span style={{ fontWeight:800 }}>€{f0(r.tot)}</span>{prv?.tot!=null&&<div style={{ fontSize:9, fontWeight:700, color:r.tot>prv.tot!'var(--red)':'var(--green)' }}>{r.tot>prv.tot!'▲':'▼'}{Math.abs(pct(r.tot,prv.tot!)).toFixed(1)}%</div>}</>:<span style={{ color:'var(--text3)' }}>—</span>}
                        </td>
                        {/* Rate versate */}
                        <td style={{ padding:'8px 10px', textAlign:'right' }}>
                          <span style={{ fontWeight:700, color:'var(--accent)' }}>{r.rateAnno>0?`€${f0(r.rateAnno)}`:'—'}</span>
                          {r.rateCount>0&&<div style={{ fontSize:9, color:'var(--text3)' }}>{r.rateCount} rata{r.rateCount>1?'e':''}</div>}
                        </td>
                        {!showBreakdownR && (
                          <td style={{ padding:'8px 10px', textAlign:'right' }}>
                      ...
                      return (
                        <tr key={r.anno} style={{ borderBottom:'1px solid var(--border)' }}>
                          ...
                          <td style={{ padding:'6px 10px', textAlign:'right' }}>
                            ...
                            {prv?.rUnit&&r.rUnit&&<div style={{ fontSize:9, color:r.rUnit>prv.rUnit?'var(--red)':'var(--green)', fontWeight:700 }}>{r.rUnit>prv.rUnit?'▲':'▼'}{Math.abs(pct(r.rUnit,prv.rUnit)).toFixed(1)}%</div>}
                          </td>
                          ...
                          <td style={{ padding:'6px 10px', textAlign:'right' }}>
                            ...
                            {prv?.aUnit&&r.aUnit&&<div style={{ fontSize:9, color:r.aUnit>prv.aUnit?'var(--red)':'var(--green)', fontWeight:700 }}>{r.aUnit>prv.aUnit?'▲':'▼'}{Math.abs(pct(r.aUnit,prv.aUnit)).toFixed(1)}%</div>}
                          </td>
                          ...
                          <td style={{ padding:'6px 10px', textAlign:'right' }}>
                            ...
                            {prv?.afUnit&&r.afUnit&&<div style={{ fontSize:9, color:r.afUnit>prv.afUnit?'var(--red)':'var(--green)', fontWeight:700 }}>{r.afUnit>prv.afUnit?'▲':'▼'}{Math.abs(pct(r.afUnit,prv.afUnit)).toFixed(1)}%</div>}
                          </td>
                        </tr>
                      );
                    })}
      ...
      {tab==='Consumi' && (
        <div ref={consumiTopRef} style={{ display:'flex', flexDirection:'column', gap:12 }}>
          ...
          {/* Toggle euro/quantità + breakdown */}
                ...
                <table style={{ width:'100%', borderCollapse:'collapse', fontSize:11 }}>
                  ...
                  <tbody>
                    {riassunto.filter(r=>r.rKwh||r.aL||r.afL).map((r,i)=>{
                      ...
                      const cell = (val: number|null, prv: number|null, col: string) => (
                        <td style={{ padding:'6px 10px', textAlign:'right' }}>
                          {val ? <><span style={{ fontWeight:700, color:col }}>{f2(val)}</span>{prv&&<div style={{ fontSize:9, fontWeight:700, color:val>prv?'var(--red)':'var(--green)' }}>{val>prv?'▲':'▼'}{Math.abs(pct(val,prv)).toFixed(1)}%</div>}</> : <span style={{ color:'var(--text3)' }}>—</span>}
                        </td>
                      );
                      return (
                        <tr key={r.anno} style={{ borderBottom:'1px solid var(--border)' }}>
                          <td style={{ padding:'6px 10px' }}><span className="tag tag-blue">{r.anno}</span></td>
                          <td style={{ padding:'6px 10px', textAlign:'right', fontWeight:600 }}>{fN(r.rKwh)}</td>
                          {cell(r.rUnit||null, prv?.rUnit||null, '#ef4444')}
                          <td style={{ padding:'6px 10px', textAlign:'right', fontWeight:600 }}>{fN(r.aL)}</td>
                          {cell(r.aUnit||null, prv?.aUnit||null, '#f97316')}
                          <td style={{ padding:'6px 10px', textAlign:'right', fontWeight:600 }}>{fN(r.afL)}</td>
                          {cell(r.afUnit||null, prv?.afUnit||null, '#3b82f6')}
                        </tr>
                      );
                    })}
                  </tbody>
          ...
          {[...consumi].reverse().map((r,i) => {
            ...
            const cardBorder = isHighlighted ? { border:'2.5px solid #f59e0b', boxShadow:'0 0 0 4px #fef3c7' } : hasErrors ? { border:'2px solid #fca5a5' } : hasWarns ? { border:'2px solid #fde68a' } : undefined;
            return (
              <div key={r.id} className="card" style={cardBorder}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    {/* Chips consumi: quantità + costo unitario */}
                    {(rKwh || aL || afL) && (
              ...
              <div key={yl} className="card">
                ...
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:10 }}>
                  <div>
                    ...
                    <p style={{ fontSize:11, color:'var(--text3)', marginTop:2 }}>{rAnno.length} rata{rAnno.length>1?'e':''} · media €{fa(avgRata)} cad.</p>
                  </div>
                  <div style={{ textAlign:'right' }}>
                ...
                {/* Alert corrispondenza rendiconto */}
                {atteso !== null && (
                ...
                {/* Lista singole rate */}
                <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
                  {rAnno.map((r:any, ri:number) => {
                    const tot=(parseFloat(r.importo_casa)||0)+(parseFloat(r.importo_box)||0)+(parseFloat(r.importo_cantina)||0);
                    // Barra di progressione rate
                    const pctBar = atteso ? Math.min(100, (rAnno.slice(0,ri+1).reduce((s:number,x:any)=>(s+(parseFloat(x.importo_casa)||0)+(parseFloat(x.importo_box)||0)+(parseFloat(x.importo_cantina)||0)),0) / atteso) * 100) : 0;
                    return (
                ...
                {/* Barra progressione se c'è atteso */}
                {atteso && (
                  <div style={{ marginTop:10 }}>
